...
export abstract class ValueConvertor<T, U> {
    ...
    static #identity = new class extends ValueConvertor<never, never> {
        convertForwards (x: never): never {
            return x;
        }

        convertBackwards (x: never): never {
            return x;
        }
    };

    /** The identity convertor. This is always the same object */
    public static identity<T> (): ValueConvertor<T, T> {
        return this.#identity;
    }

    public static create<T, U> (
        forwards: (a: T) => U, backwards: (b: U) => T,
    ): ValueConvertor<T, U> {
        const res = new (ValueConvertor as any)() as ValueConvertor<T, U>;
        res.convertForwards = forwards;
        res.convertBackwards = backwards;
        return res;
    }
}
...
export function convertRangeForwards (
    convertor: NumericValueConvertor, range: ReadonlyNumberRange) {
    return range.copy()
        .modifyStart(convertor.convertForwards)
        .modifyEnd(convertor.convertForwards);
}

export function convertRangeBackwards (
    convertor: NumericValueConvertor, range: ReadonlyNumberRange) {
    return range.copy()
        .modifyStart(convertor.convertBackwards)
        .modifyEnd(convertor.convertBackwards);
}

export const logTransform: NumericValueConvertor = ValueConvertor.create(
    a => Math.log(a), b => Math.exp(b),
);
export const expTransform: NumericValueConvertor = logTransform.inverted();

export function powTransform (pow: number): NumericValueConvertor {
    return ValueConvertor.create(a => a ** pow, b => b ** (1 / pow));
}
...
export class LinearValueConvertor extends ValueConvertor<number, number> {
     ...
     *
     * If {@link applyOffsetFirst} is true, convertForwards(x) returns `x *
     * factor + offset`, otherwise, it returns `(x + offset) * factor`
     */
    ...
    static fittingRangeTo (
        sourceRange: ReadonlyNumberRange,
        destRange: ReadonlyNumberRange,
    ): NumericValueConvertor {
        const factor = destRange.length / sourceRange.length;
        return new LinearValueConvertor(
            factor, destRange.start - sourceRange.start * factor);
    }

    static normalizing (sourceRange: ReadonlyNumberRange): NumericValueConvertor {
        return LinearValueConvertor.fittingRangeTo(
            sourceRange, new NumberRange(0, 1));
    }

    static fittingRangeToAfter (
        sourceRange: ReadonlyNumberRange, destRange: ReadonlyNumberRange,
        transform: NumericValueConvertor,
    ): NumericValueConvertor {
